...
$$.HashSetIterator = {"":
 ...
 hasNext$0$bailout: function(state, t1, t2) {
  if ($.geB(t1, $.get$length(t2)))
    return false;
  if ($.index(t2, this._nextValidIndex) === $.CTC16)
    this._advance$0();
...
$$.TimeTracker = {"":
 ["tasks?", "autoSave", "tasksDiv?", "body", "activeTasks"],
 "super": "Object",
 saveAll$0: function() {
 ...
 setState$1: function(active) {
  var t1 = this.activeTasks;
  if (active)
    this.activeTasks = t1 + 1;
  else
    this.activeTasks = t1 - 1;
  t1 = this.activeTasks > 0;
  var t2 = this.body;
  if (t1) {
    $.add$1(t2.get$classes(), 'working');
    t2.get$classes().remove$1('idle');
  } else {
    $.add$1(t2.get$classes(), 'idle');
    t2.get$classes().remove$1('working');
  }
},
...
$$.Task = {"":
 ["seconds=", "name=", "taskDiv", "nameInput", "timeInput?", "tasksDiv?", "timeTracker?", "timer", "working"],
 "super": "Object",
 remove$0: function() {
  this.stop$0();
  this.taskDiv.remove$0();
},
 start$0: function() {
  if (this.working !== true) {
    this.timeInput.set$disabled(true);
    this.timer = $._TimerFactory_Timer$repeating(1000, new $.Task_start_anon($.Date_Date$now().subtract$1($.Duration$(0, 0, 0, this.seconds, 0)), this));
    this.working = true;
    this.timeTracker.setState$1(true);
  }
},
 stop$0: function() {
  if (this.working === true) {
    this.timer.cancel$0();
    this.timeInput.set$disabled(false);
    this.working = false;
    this.timeTracker.setState$1(false);
  }
},
 formatTimeString$1: function(seconds) {
  var h = $.toInt($.div(seconds, 3600));
  var m = $.toInt($.div($.sub(seconds, $.mul($.mul(h, 60), 60)), 60));
  var s = $.mod(seconds, 60);
  s = $.geB(s, 10) ? s : '0' + $.S(s);
  m = $.geB(m, 10) ? m : '0' + $.S(m);
  return $.S(h) + ':' + $.S(m) + ':' + $.S(s);
},
 parseTimeString$1: function(time) {
  var parts = $.split(time, ':');
  if (!$.eqB($.get$length(parts), 3))
    return -1;
  var h = $.int_parse($.index(parts, 0));
  if ($.isNaN(h) === true || $.ltB(h, 0))
    return -1;
  var m = $.int_parse($.index(parts, 1));
  if ($.isNaN(m) === true || $.ltB(m, 0) || $.gtB(m, 60))
    return -1;
  var s = $.int_parse($.index(parts, 2));
  if ($.isNaN(s) === true || $.ltB(s, 0) || $.gtB(s, 60))
    return -1;
  return $.add($.add($.mul($.mul(h, 60), 60), $.mul(m, 60)), s);
},
 Task$3: function(timeTracker, name$, seconds) {
  this.taskDiv = $.document().query$1('#task-template').clone$1(true);
  var t1 = this.taskDiv;
  t1.get$attributes().remove$1('id');
  this.nameInput = t1.query$1('.name');
  this.timeInput = t1.query$1('.time');
  var t2 = this.name;
  var t3 = this.nameInput;
  t3.set$value(t2);
  t2 = this.formatTimeString$1(this.seconds);
  var t4 = this.timeInput;
  t4.set$value(t2);
  $.add$1(this.timeTracker.get$tasksDiv().get$elements(), t1);
  $.add$1(t3.get$on().get$keyUp(), new $.anon3(this));
  $.add$1(t4.get$on().get$keyUp(), new $.anon4(this));
  $.add$1(t1.query$1('.start').get$on().get$click(), new $.anon5(this));
  $.add$1(t1.query$1('.stop').get$on().get$click(), new $.anon6(this));
  $.add$1(t1.query$1('.delete').get$on().get$click(), new $.anon7(this));
}
};
...
$$._Timer = {"":
 ...
 _Timer$repeating$2: function(milliSeconds, callback) {
  this._handle = $._window().setInterval$2(new $.anon10(this, callback), milliSeconds);
},
 _Timer$2: function(milliSeconds, callback) {
  this._handle = $._window().setTimeout$2(new $.anon9(this, callback), milliSeconds);
}
};
...
$$.anon1 = {"":
 ...
 "super": "Closure",
 call$1: function(event$) {
  if ($.window().confirm$1('Do you really want to clear all tasks?') === true) {
    var t1 = this.this_2;
    $.forEach(t1.get$tasks(), new $.anon8());
    $.clear(t1.get$tasks());
    $.add$1(t1.get$tasks(), $.Task$(t1, '', 0));
  }
}
...
$$.anon3 = {"":
 ...
 "super": "Closure",
 call$1: function(event$) {
  var t1 = $.callTypeCast(event$.get$srcElement(), 'is$InputElement').get$value();
  this.this_0.set$name(t1);
}
};
...
$$.anon4 = {"":
 ...
 "super": "Closure",
 call$1: function(event$) {
  var timeInput = event$.get$srcElement();
  var t1 = this.this_1;
  var s = t1.parseTimeString$1(timeInput.get$value());
  if (!$.eqB(s, -1)) {
    t1.set$seconds(s);
    timeInput.get$classes().remove$1('error');
  } else
    $.add$1(timeInput.get$classes(), 'error');
}
};
...
$$.anon7 = {"":
 ...
 "super": "Closure",
 call$1: function(event$) {
  if ($.window().confirm$1('Do you really want to remove this task?') === true) {
    var t1 = this.this_4;
    t1.remove$0();
    var t2 = t1.get$timeTracker().get$tasks();
    $.removeRange(t2, $.indexOf$1(t2, t1), 1);
  }
}
};
...
$$.Task_start_anon = {"":
 ...
 "super": "Closure",
 call$1: function(timer) {
  var t1 = $.Date_Date$now().difference$1(this.started_1).get$inSeconds();
  var t2 = this.this_0;
  t2.set$seconds(t1);
  t1 = t2.formatTimeString$1(t2.get$seconds());
  t2.get$timeInput().set$value(t1);
}
};
...
$$.TimeTracker_saveAll_anon = {"":
 ...
 call$1: function(task) {
  var t1 = this.data_0;
  if (t1.containsKey$1(task.get$name()) === true) {
    var t2 = task.get$name();
    var t3 = task.get$seconds();
    $.indexSet(t1, t2, $.add($.index(t1, t2), t3));
  } else
    $.indexSet(t1, task.get$name(), task.get$seconds());
}
};
...
$.Duration$ = function(days, hours, minutes, seconds, milliseconds) {
  ...
  return new $.Duration(t2 + milliseconds);
};
...
$.TimeTracker$ = function() {
  var t1 = new $.TimeTracker([], null, $.document().query$1('#tasks'), $.document().query$1('body'), 0);
  t1.TimeTracker$0();
  return t1;
};
...
$.Task$ = function(timeTracker, name$, seconds) {
  var t1 = new $.Task(seconds, name$, null, null, null, null, timeTracker, null, false);
  t1.Task$3(timeTracker, name$, seconds);
  return t1;
};
...
$.CTC20 = new Isolate.$isolateProperties.ConstantMap(0, {}, Isolate.$isolateProperties.CTC1);
...
$.CTC26 = 'frozen class set cannot be modified';
...
$.CTC39 = 'Cannot removeRange on immutable List.';
...
$.CTC40 = 'structured clone of Blob';
...
//@ sourceMappingURL=time_tracker.dart.js.map